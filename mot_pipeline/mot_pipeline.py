...
def _log_assoc_results(assoc_out, detections):
    """Detailed logging of association outcome, including explicit class updates."""
    # per-camera counts
    ...
    # class confirmations / updates
    updates = 0
    keeps = 0
    for c in assoc_out.class_confirmations:
        prev = (c.prev_class or "").lower()
        new = (c.confirmed_class or "").lower()
        updated = (new and new != prev)

        # Always log a concise confirmation line
        logger.info(
            f"[CONFIRM] T{c.track_id} via cam{c.camera_id}: SDIoU={c.sdiou:.2f} {prev or 'none'} -> {new or 'none'}"
            f"{' (UPDATED)' if updated else ''}"
        )

        # Extra explicit line when a change happens
        if updated:
            updates += 1
            logger.info(f"[CLASS-UPDATE] T{c.track_id}: '{prev or 'none'}' -> '{new}' (cam{c.camera_id}, SDIoU={c.sdiou:.2f})")
        else:
            keeps += 1
            logger.info(f"[CLASS-KEEP]   T{c.track_id}: '{prev or 'none'}' (cam{c.camera_id}, SDIoU={c.sdiou:.2f})")

    if updates or keeps:
        logger.info(f"[SUMMARY] Class updates: {updates}, kept: {keeps}")

    # merges
    for m in assoc_out.merges:
        sdiou_str = ", ".join(f"{s:.2f}" for s in m.sdiou_values)
        logger.info(
            f"[MERGE] cam{m.camera_id} det#{m.det_index} "
...
def main(args=None):
    ...
    # (You set class_map=None in association call to force camera overwrite when SDIoU ≥ threshold)
    ...
    try:
        while rclpy.ok():
            ...
            assoc_out = run_measurement_association_sdiou(
                ...
                class_map=None,                    # <-- force overwrite when SDIoU high & classes differ
            ...
            # logs (counts, confirmations, merges + explicit class updates)
            _log_assoc_results(assoc_out, detections)

            # -------- overlays + H-stack save by LiDAR timestamp --------
            # Your TrackObject3dList exposes an epoch string property directly.
            lidar_ts = track_list.epoch_time  # (string/float is fine for filename)
            ...
            render_hstack_and_save(
                ...
                draw_wireframe_3d=False,  # True = draw 3D wireframes too
                cam_order=cam_ids_sorted
            )

            # (Optional) also save individual per-camera overlays
