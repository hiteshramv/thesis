# Two-stage MOT:
#   Stage-1: 3D association using scaled distance cost
...
from motion_model_non_linear import make_model, correct_new_angle_and_diff
import logging
...
def scaled_dist(a: Dict[str, Any], b: Dict[str, Any]) -> float:
    ...
    # compromise weights: x,y strong; z medium; size weak
    w_pos  = np.array([1.0, 1.0, 0.4], dtype=np.float32)
    w_size = np.array([0.2, 0.2, 0.1], dtype=np.float32)
...
# ============================================================
# NonLinear EKF wrapper
# ============================================================
 
class NonLinearEKF:
    """Small EKF wrapper around the nonlinear motion-model classes."""
 
    def __init__(self, model_name: str, obj3d: Any, dt: float = 0.1):
        #has_velo = getattr(obj3d, "speed", None) is not None
        self.model = make_model(model_name, has_velo=False, dt=dt)
 
        self.dim_x = int(self.model.state_dim)
        self.dim_z = int(self.model.meas_dim)
 
        self.x = self.model.get_init_state(obj3d)
        self.P = self.model.get_init_cov_p()
        self.Q = self.model.get_process_noise_q()
        self.R = self.model.get_mea_noise_r()
        self.F = np.eye(self.dim_x, dtype=float)
        self.H = self.model.get_mea_state_h(self.x)
 
    def predict(self, dt: float) -> None:
        self.model.dt = float(max(1e-3, dt))
        self.F = self.model.get_transition_f(self.x)
        self.Q = self.model.get_process_noise_q()
 
        self.x = self.model.state_transition(self.x)
        self.P = self.F @ self.P @ self.F.T + self.Q
        self.x = self.model.warp_state_yaw_to_pi(self.x)
 
    def update(self, z: np.ndarray) -> None:
        z = np.asarray(z, dtype=float).reshape(self.dim_z, 1)
        state_info = self.model.state_to_measure(self.x)
 
        res = z - state_info
        res = self.model.warp_res_yaw_to_pi(res)
 
        # H and R are constant (linear measurement model, fixed noise).
        # They were precomputed in __init__ — no need to recompute here.
...
class Track:
    ...
    def __init__(self, obj3d: Any, ts: float, motion_model: str):
        ...
        # Nonlinear EKF (state size depends on motion model)
        self.kf = NonLinearEKF(motion_model, obj3d, dt=0.1)
    ...
    def update_with_3d(self, obj3d: Any):
        ...
        new_yaw = float(z[3])
        new_yaw_corr, _ = correct_new_angle_and_diff(float(self.kf.x[3,0]), new_yaw)
        z[3] = new_yaw_corr
...
class KalmanMultiObjectTracker:
    """
    Stage-1: 3D association (scaled distance cost) between 3D detections and existing tracks
    ...
    def update_two_stage(self,
        ...
        # ----------------------------------------------------
        # Stage-1: 3D association (scaled distance cost)
        # ----------------------------------------------------
        ...
        # candidate_matches: local track index -> list of (cam_id, det_local_index)
        ...
        for cam_id, res in assoc_debug.items():
            ...
            m2, _, _ = filter_matches(m2, u2d_det, u2d_trk, sim2d, self.iou2d_thresh)
...
#     def __init__(self, model_name: str, obj3d: Any, dt: float = 0.1):
#         #has_velo = getattr(obj3d, "speed", None) is not None
#         self.model = make_model(model_name, has_velo=None, dt=dt)
...
#         # ----------------------------------------------------
#         # Stage-1: Paper association (scaled distance cost)
#         # ----------------------------------------------------
#         track_boxes = [trk.predicted_box3d_dict() for trk in self.tracks]
#         det_boxes = [self._obj3d_to_boxdict(obj) for obj in dets3d]
...
#             vals2 = sim2d[sim2d > 0]
#             logger2.info(f"[mot2d frame={self.frame_count} cam={cam_id}] sim2d {_stats(vals2)}")
